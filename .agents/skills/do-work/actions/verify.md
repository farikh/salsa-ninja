...
> **Part of the do-work skill (Salsa Ninja fork).** Invoked when routing determines the user wants to verify the quality of captured requests. Evaluates REQ files against their originating User Request (UR) to find gaps.
>
> **Fork addition:** After the standard verification, also cross-checks beads to ensure all are properly closed.
...
2. Check `do-work/` (queue) and `do-work/archive/` for each
...
## Beads Cross-Check (Fork Addition)

After the standard verification report, also check beads consistency:

1. **Run:** `cd app && bd list` to get all beads
2. **For each completed REQ:** verify its corresponding bead is closed
3. **For each open bead:** verify it has a matching pending or in-progress REQ
4. **Report orphans:**
   - Beads with no matching REQ → "Orphan bead: app-xyz (no REQ match)"
   - Completed REQs with open beads → "Desync: REQ-NNN completed but bead app-xyz still open"
5. **Offer fixes:** close orphan beads, sync desynchronized state

Maximum 2 verification → fix loops before escalating to user.
