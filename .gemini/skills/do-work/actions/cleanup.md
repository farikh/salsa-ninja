...
> **Part of the do-work skill (Salsa Ninja fork).** Invoked when routing determines the user wants to tidy the archive, or automatically at the end of the work loop. Consolidates loose files and ensures the archive is well-organized.
>
> **Fork addition:** Also detects orphaned beads and sync issues.
...
2. For each REQ ID in the array, check if it exists with `status: completed` in ANY of these locations:
   ...
   - `do-work/` root (just completed, not yet moved)
3. If **ALL** REQs are completed:
   - Gather any loose completed REQ files from `do-work/archive/` root into the UR folder
   - Gather any completed REQ files from `do-work/` root into the UR folder
...
- Touch files in `do-work/` root (the queue) — those are the work action's responsibility
- Archive UR folders that still have pending/in-progress REQs
- Process any REQ files (use the work action for that)

---

## Pass 4: Bead Orphan Detection (Fork Addition)

After the standard three passes, check bead consistency:

1. **Run:** `cd app && bd list` to get all beads
2. **For each archived REQ with a `bead_id`:** verify the bead is closed
3. **For open beads with no matching pending REQ:** flag as orphan
4. **Report:**
   ```
   Bead check:
     - Synced: 12 beads match archived REQs
     - Orphans: app-xyz (no REQ), app-abc (no REQ)
     - Desync: app-def open but REQ-005 completed
   ```
5. **Offer to close** orphan beads (with user confirmation)

**Note:** Beads created outside the do-work workflow (e.g., manual `bd create`, deferred questions) are expected to have no REQ match. Only flag beads that have a `bead_id` reference pattern matching REQ files.

### Path Override

All `do-work/` paths are relative to `app/`. The directory lives at `app/do-work/`, not the project root.
